...
bool IMU_WriteData(uint8_t reg, uint8_t data)
{
    ...
    esp_err_t ret = i2c_master_write_to_device(0, IMU_I2C_ADDRESS, buf, 2, pdMS_TO_TICKS(100));
    return (ret == ESP_OK);
}
...
{
    if (!is_initialized || data == NULL) return false;
    esp_err_t ret = i2c_master_write_read_device(0, IMU_I2C_ADDRESS, &reg, 1, data, 1, pdMS_TO_TICKS(100));
    return (ret == ESP_OK);
}
...
{
    if (!is_initialized || data == NULL) return false;
    esp_err_t ret = i2c_master_write_read_device(0, IMU_I2C_ADDRESS, &reg, 1, data, length, pdMS_TO_TICKS(100));
    return (ret == ESP_OK);
}
