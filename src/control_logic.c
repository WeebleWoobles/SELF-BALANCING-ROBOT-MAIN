...
void ControlLogic_Update(const StateEstimate_t* state, const UserInput_t* input, ControlOutput_t* output)
{
    ...
    else if (pid_out < -output_limit) pid_out = -output_limit;
    ...
    // clamp outputs
    if (output->motor_left > output_limit) output->motor_left = output_limit;
    else if (output->motor_left < 0) output->motor_left = 0;

    if (output->motor_right > output_limit) output->motor_right = output_limit;
    else if (output->motor_right < 0) output->motor_right = 0;
...
void ControlLogic_SetUserInput(const UserInput_t* input)
{
    ...
    else user_input = (UserInput_t){0}; // reset if NULL
}
